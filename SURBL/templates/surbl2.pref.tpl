...
<form action="plugin.page.php?plugin=surbl&action=page2&do=save&sid={$sid}" method="post" onsubmit="spin(this)">	
	<fieldset>
		<legend>{lng p="surbl_list"}</legend>
	
		<table width="90%">
			<tr>
				<td align="left" rowspan="3" valign="top" width="40"><img src="{$tpldir}images/antispam_dnsbl.png" border="0" alt="" width="32" height="32" /></td>
				<td class="td1" width="220">{lng p="surbl_list"}:</td>
				<td class="td2">
					<textarea style="width:100%;height:200px;" name="surbl_list">{text value=$surbl_list allowEmpty=true}</textarea>
